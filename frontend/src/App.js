...
import { fetchTasks, createTask, updateTask, analyzeTask } from './api/tasks';
...
function App() {
  ...
  const handleAnalyze = async (id) => {
    const result = await analyzeTask(id);
    setTasks((prev) =>
      prev.map((t) =>
        t.id === id
          ? { ...t, category: result.category, subtasks: result.subtasks }
          : t
      )
    );
  };
